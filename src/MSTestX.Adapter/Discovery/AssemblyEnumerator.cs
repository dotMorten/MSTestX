...
internal class AssemblyEnumerator : MarshalByRefObject
{
    ...
    internal ICollection<UnitTestElement> EnumerateAssembly(string assemblyFileName, out ICollection<string> warnings)
    {
        ...
        // a breaking change.
        // MSTESTX: USE REFLECTION INSTEAD TO ACCESS PROPERTY
        PropertyInfo? p = typeof(DataRowAttribute).GetProperty("TestIdGenerationStrategy", BindingFlags.Static | BindingFlags.NonPublic);
        p?.SetValue(null, testIdGenerationStrategy);
