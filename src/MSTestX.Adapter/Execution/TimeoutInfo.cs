...
internal readonly struct TimeoutInfo
{
    ...
    public static TimeoutInfo FromTimeoutAttribute(TimeoutAttribute timeoutAttribute)
        => new(
            ...
            IsCooperativeCancellationSet(timeoutAttribute) // MSTESTX
                ? timeoutAttribute.CooperativeCancellation
                : MSTestSettings.CurrentSettings.CooperativeCancellationTimeout);

    // MSTESTX:
    private static bool IsCooperativeCancellationSet(TimeoutAttribute timeoutAttribute)
    {
        var prop = typeof(TimeoutAttribute).GetProperty("IsCooperativeCancellationSet", System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.NonPublic);
        var isset = (bool?) prop?.GetValue(timeoutAttribute, null) ;
        return isset == true;
    }
